    ...
    match op {
        ...
        DBG_ADVANCE_LINE => {
            if let Some((adv, bytes)) = decode_leb128_s(&ops[off..]) {
                off += bytes as usize;
                ctx.file_line = (ctx.address as i64 + adv) as u32;
                RunResult::Ok(off - pc)
            } else {
                RunResult::Err
            }
        ...
        // Special operators.
        //
        // All operators that is bigger than or equal to
        // DBG_FIRST_SPECIAL are special operator.  Special operators
        // changes both the line number and address of the virtual
        // machine, and emit a new row.
        _ => {
            ...
            ctx.file_line += line_delta as u32;
