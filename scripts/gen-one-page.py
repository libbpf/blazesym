...
This script parses HTML files generated by rustoc and generates a
single page API document.
...
def replace_text(src, replace, start, stop):
    ...
    if start[0] == stop[0]:
        lineno = start[0]
        lines[lineno] = lines[lineno][:start[1]] + replace + lines[lineno][stop[1] + 6:]
    else:
        lines[start[0]] = lines[start[0]][:start[1]]
        lines[stop[0]] = lines[stop[0]][stop[1] + 6:]
        ...
        if hasattr(self, 'replace_depth'):
            if self.replace_depth > 0:
                ...
                if not hasattr(self, 'replaces'):
                    self.replaces = []
                    pass
                self.replaces.append((self.replacing, self.replace_start, self.getpos()))
                pass
        elif hasattr(self, 'depth'):
            if self.depth > 0:
                self.depth -= 1
                ...
                del self.depth
                if self.doc_type == 'crate':
                    content = self.get_doc_range(self.start_pos, self.getpos())
                    print('%s' % content)
                    pass
                elif self.doc_type == 'fn':
                    print('%s\n</section>' % self.get_doc_range(self.start_pos, self.getpos()))
